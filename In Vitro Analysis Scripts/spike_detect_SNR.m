function  result=spike_detect_SNR(traces,up_threshold, down_threshold)
...
for neuron=1:size(traces,2)
 current_trace = detrend(traces(:,neuron));
              event.idx=[];
            event.amplitude=[];
            event.snr=[];
            ...
            % Trace consists of points higher than the movmean or equal
            % to the movmean if the original point was lower than the
            % movment
...
%            result.roi(roi_idx).noise_idx = noise_idx_list;
            ...
            % Use the noise_idx_list and set them to nan.
            % Essentially remove the determined noise points
...
%%%%%%%%%%
    % A shift in the difference of the upper trace
     pre_d_trace = d_u_f_trace;
            ...
            figure;
            plot(d_u_f_trace);
            title(['d_u_f_trace neuron ' num2str(neuron)])
            
            figure;
            plot(denoise_trace);
            title(['denoise trace neuron ' num2str(neuron)])
            end
%%%%%%%%%%%%%
 trace_val=pre_d_trace;
    % Here is thresholding (decision point here)
   up_idx_list = find(pre_d_trace>(nanmean(d_u_f_trace)+event_parameter.up_threshold_value));

    % DEBUG Check how the first attempt worked
    if debug == 1 & neuron == 5
    figure;
    plot(pre_d_trace);
    hold on;
    plot(repmat(event_parameter.up_threshold_value, length(pre_d_trace)), '.r');
    title(['Pre_d_trace with threshold #' num2str(neuron)]);
    end
 ...
 for up_idx=up_idx_list'
  ...
  if isempty(valnear); vthres=0; else;  vthres=max(trace_val(up_idx_list(valnear)));end
           ...
           current_signal_intensity = max(peak_intensity-pre_peak_intensity_1,peak_intensity-pre_peak_intensity_2);
            ...
            event.roaster(event.idx) = 1;   % Here is the spike index
...
result.spike_snr{neuron} =  event.snr ;
result.spike_amplitude{neuron} = event.amplitude ;
result.spike_idx{neuron} =  event.idx  ;
...
function noise_idx_list = find_noise_idx(current_trace,trace_moving_window,noise_moving_window,noise_threshold,noise_extension)
    ...
    % Here is a first attempt to find points that are considered noise by
    ...
    % connect noise index
    % not 100%, I think the connection here is to include idx's that are
    % within a small gap, in this case it is [1-noise_extension].
